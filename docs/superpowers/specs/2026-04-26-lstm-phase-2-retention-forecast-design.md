...
- prior: `PriorBundle.retention.dN: EmpiricalDist` (`{p10, p50, p90}`) → 기존 `betaBinomialModel.priorFromEmpirical(empirical, effectiveN)` 으로 `Beta(α₀, β₀)` 변환 (재사용)
...
import type { BetaParams, BinomialObs } from "./beta-binomial"  // 기존 타입 재사용
import type { EmpiricalDist } from "./types"                     // 기존 타입 재사용
...
export function retentionForecast(args: {
  ...
  priorEffectiveN: number   // PriorBundle.effectiveN 그대로 전달
  priorWeight?: number      // default 1.0
...
`betaFromMoments`는 신규 작성하지 않음 — 기존 `betaBinomialModel.priorFromEmpirical(empirical, effectiveN)` 직접 재사용.
...
| prior degenerate (`p90 ≤ p10`, `p50 ∉ (0,1)`, variance ≥ μ(1−μ), 또는 implied α+β < 1) | throw `DegenerateDistributionError` (기존 `priorFromEmpirical` 가드 위임) |
...
| `priors`가 비-finite 또는 부정합 | `priorFromEmpirical` 가드에 위임 |

기존 `bayesian-stats/`의 `DegenerateDistributionError` 클래스를 재사용 (prior 변환은 `priorFromEmpirical`에 위임하므로 자동 throw). 신규 에러 클래스(`InvalidObservationError`, `InvalidPriorWeightError`, `NonDecreasingCurveError`, `MaxDayOutOfRangeError`)는 throw하는 파일에 정의.

**제거된 신규 에러**: `betaFromMoments` 폐기에 따라 `InvalidPriorMomentsError`도 불필요 — degenerate 검출은 모두 기존 `DegenerateDistributionError`로 단일화.
...
- prior `Beta(2, 8)`, obs `k=80/n=100`, w=1 → posterior `Beta(82, 28)`, p50 ≈ 0.7455
- priorWeight=0.5: prior `Beta(2, 8)`, obs `k=80/n=100` → posterior `Beta(81, 24)`
- n=0: posterior = prior (단, weighted prior 적용 — α₀×w, β₀×w)
- k > n → throw `InvalidObservationError`
- priorWeight ≤ 0 → throw `InvalidPriorWeightError`

**Prior 변환은 기존 `priorFromEmpirical`을 직접 호출**해서 검증 — degenerate input(p10=p90, p50=0 등)은 기존 테스트에서 이미 커버되므로 본 PR에서 중복 테스트하지 않음.

**`fitPowerLaw`**:
- 입력 `[(1, 0.5), (7, 0.3), (30, 0.15)]` → b ≈ 0.35, a ≈ 0.53 (loose `toBeCloseTo(value, 1)`)
- 입력 `[(1, 0.3), (7, 0.5), (30, 0.7)]` → throw `NonDecreasingCurveError`

**`extrapolatePowerLawCurve`**:
- 입력 `fit={a:0.5, b:0.36}, maxDay=1095, floor=0.01` → length 1095, 마지막 값 ≥ 0.01
- maxDay > 1095 또는 ≤ 0 → throw `MaxDayOutOfRangeError`

**Edge cases**: 위 §5 표 5개 모두 별도 테스트.

### 6.2 Integration 테스트 (`__tests__/retention-forecast.test.ts`)

골든 케이스 1개 — `poko_merge`(gameKey)가 속한 `Merge:JP`(genre:region) bundle 사용:
- obs: D1=70/100, D7=35/100, D30=15/100
- prior: `getPrior({genre:"Merge",region:"JP"})!.retention` 실제 값 (장르 단위, gameKey 단위 아님)
- effectiveN: `getPrior(...)!.effectiveN` 그대로 전달

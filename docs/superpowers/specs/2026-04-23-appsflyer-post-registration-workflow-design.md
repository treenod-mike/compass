...
- **`feat/bayesian-stats-engine` 브랜치 off-limits**: 별도 세션 작업, 본 spec 은 건드리지 않음.
...
export const CohortSummarySchema = z.object({
  ...
  cohorts: z.record(z.string(), z.object({    // key = installDate (YYYY-MM-DD)
    n: z.number().int().nonnegative(),         // 해당 cohort 총 설치수

...
**포함 안 함** (§10 상세):
다장르·다리전 Prior, 다 vendor MMP, 다 게임 MMP sync, 추가 지표(ARPDAU/CPI/LTV), MCMC 엔진, request-time recomputation, Match League / Weaving Fairy / Dig Infinity의 per-game prior-posterior (이들은 mock 유지).
...
Match League / Weaving Fairy / Dig Infinity는 registry에 없음 → prior-posterior chart는 mock 계속 사용.
...
- Match League, Weaving Fairy, Dig Infinity의 per-game prior-posterior (mock 유지, registry 제외)

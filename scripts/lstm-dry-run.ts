...
async function main() {
  ...
  const out: unknown[] = []
  // Per-app try/catch mirrors the cron route's soft-fail contract: a single
  // game's failure must not abort the whole batch.
  for (const app of targets) {
    try {
      ...
      out.push({
        ...
        // retentionCurve is day 1..1095 → index 29 = day 30
        day30RetentionP50: result.retentionCurve[29]?.p50,
        // revenueForecast is day 1..365 → index 89 = day 90
        day90RevenueP50: result.revenueForecast[89]?.revenueP50,
        ...(process.env.LSTM_DRY_FULL === "1"
          ? { retentionCurve: result.retentionCurve, revenueForecast: result.revenueForecast }
          : {}),
      })
    } catch (err) {
      out.push({ gameId: app.appId, status: "error", message: String(err) })
    }
  }
